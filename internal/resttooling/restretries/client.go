...
func (rt *retryableRoundTripper) RoundTrip(originalReq *http.Request) (*http.Response, error) {
	...
	action := func(_ context.Context) (err error) {
		...
		// if the request was successful (from a network
...
// client.
// return either nil or NewPermanentError to stop trying and exit early.
// return error that is not IsPermanentError to signal that retry would take place.
