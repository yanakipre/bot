...
//
// Timeout specifies time limit for individual attempt to get a response from remote server,
// within a call to the round tripper. This means, that if the round tripper uses WithRetries,
// then there will me as many attempts as WithRetries permits, each with its own timeout.
//
// The timeout includes connection time, any
// redirects if permitted, and reading the response body. The timer remains
// running after Get, Head, Post, or Do return and will
// interrupt reading of the Response.Body.
func WithTimeout(timeout time.Duration) Option {
	return func(cfg optionsConfig) optionsConfig {
		cfg.Timeout = func(c *http.Client) *http.Client {
			c.Transport = resttimeouts.New(c.Transport, resttimeouts.Cfg{Timeout: timeout})
			return c
		}
		return cfg
	}
...
func WithLogging(clientName string) Option {
	return func(cfg optionsConfig) optionsConfig {
		cfg.Logging = func(c *http.Client) *http.Client {
			c.Transport = roundtripper.LoggingRoundTripper(c.Transport, clientName)
			return c
		}
		return cfg
	}
...
// WithHTTPHandler causes all incoming requests to be handled using the given handler.
// No actual HTTP requests will be made.
