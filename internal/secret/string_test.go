...
func TestStringString(t *testing.T) {
	...
	assert.NotContains(
		t,
		fmt.Sprintf("%s", secret.NewString(secretValue)),
		secretValue,
		"secret is not masked when formatting",
	)
	assert.NotContains(
		t,
		fmt.Sprintf("%v", secret.NewString(secretValue)),
		secretValue,
		"secret is not masked when formatting",
	)
	assert.NotContains(
	...
	assert.NotContains(
		t,
		fmt.Sprintf("%q", secret.NewString(secretValue)),
		secretValue,
		"secret is not masked when formatting",
	)
}
...
func TestStringReflect(t *testing.T) {
	...
	for i := 0; i < typ.NumField(); i++ {
		assert.NotEmpty(
			t,
			typ.Field(i).PkgPath,
			fmt.Sprintf("field %s is exported", typ.Field(i).Name),
		)
	}
}
...
func TestStringUnmarshalJSON(t *testing.T) {
	...
	require.NoError(
		t,
		json.Unmarshal([]byte(fmt.Sprintf("{\"password\": \"%s\"}", secretValue)), &foo),
	)
